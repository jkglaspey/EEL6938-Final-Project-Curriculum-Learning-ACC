...
# Create 3 noisy sine-based speed profiles with increasing difficulty
def generate_and_save_profiles():
    t = np.arange(DATA_LEN)

    profiles = {
        "easy":     10 + 0.2 * np.sin(0.01 * t) + 0.1 * np.random.randn(DATA_LEN),
        "medium":   10 + 1.0 * np.sin(0.05 * t) + 0.5 * np.random.randn(DATA_LEN),
        "difficult":10 + 3.0 * np.sin(0.10 * t) + 1.0 * np.random.randn(DATA_LEN),
    }

    for level, speeds in profiles.items():
        df = pd.DataFrame({"speed": speeds})
        df.to_csv(f"profile_{level}.csv", index=False)
        print(f"[INFO] Saved profile_{level}.csv")

    return {k: np.cumsum(v) for k, v in profiles.items()}, profiles

LEAD_POSITION_PROFILES, LEAD_SPEED_PROFILES = generate_and_save_profiles()
...
def calc_error(current_speed, lead_speed, distance_to_lead, jerk, accel):

    # Speed error (squared to penalize large deviations)
    speed_error = abs(current_speed - lead_speed)
...
class TrainEnv(gym.Env):
    ...
    def __init__(self, lead_speeds, lead_positions, schedule_fn, chunk_size, delta_t=1.0):
        super().__init__()

        # Store speed/position metrics
        self.lead_speeds = lead_speeds
    ...
    def reset(self, seed=None, options=None):
        ...
        speed_chunks = chunk_into_episodes(self.lead_speeds[difficulty], self.chunk_size)
        ...
        ep_idx = np.random.randint(0, len(speed_chunks))
        self.current_speed_profile = speed_chunks[ep_idx]
        self.current_position = lead_chunks[ep_idx]
        self.episode_len = len(self.current_speed_profile)

        # Reset episode specific metrics
        self.step_idx = 0
        self.current_speed = self.current_speed_profile[self.step_idx]
        self.ref_speed = self.current_speed_profile[self.step_idx]
        self.prev_accel = 0.0
...
class TestEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # Use ALL episodes
        self.idx = 0
        self.current_speed = self.full_data[self.idx]
        self.prev_accel = 0.0
...
def main():
    ...
    log_dir = os.path.join(
        args.output_dir,
        f"schedule-{args.curriculum_type}"
    )
    os.makedirs(log_dir, exist_ok=True)
    ...
    episodes_list = chunk_into_episodes(LEAD_SPEED_PROFILES["difficult"], chunk_size)
    ...
    def make_train_env():
        return TrainEnv(
            lead_speeds=LEAD_SPEED_PROFILES,
    ...
    for _ in range(len(test_lead_pos)):
    ...
    def compute_metrics(start, end):
        ...
        mean_jerk = np.mean(j)
        var_jerk = np.var(j)
        mae = mean_absolute_error(lead, pred)
        rmse = np.sqrt(mean_squared_error(lead, pred))
        r2 = r2_score(lead, pred)
    ...
    section_len = len(test_lead_pos) // 3
    sections = {
    ...
    plt.plot(lead_speeds, label="Lead Vehicle Speed", linestyle="-")
    plt.plot(predicted_speeds, label="Ego Vehicle Speed", linestyle="-.", alpha=0.9)
    plt.xlabel("Timestep")
    plt.ylabel("Speed (m/s)")
    plt.title("Ego vs Lead Vehicle Speed Tracking")
    plt.legend()
    plt.grid(True, alpha=0.3)
    for boundary in [1200, 2400]:
        plt.axvline(x=boundary, color='gray', linestyle='--', linewidth=1, alpha=0.8)
    ...
    # Plot all difficulty profiles
    plt.figure(figsize=(12, 6))
    plt.plot(LEAD_SPEED_PROFILES["difficult"], label="Difficult Profile", linestyle="-", linewidth=2)
    plt.plot(LEAD_SPEED_PROFILES["medium"], label="Medium Profile", linestyle="-.", linewidth=2)
    plt.plot(LEAD_SPEED_PROFILES["easy"], label="Easy Profile", linestyle="--", linewidth=2)
    plt.xlabel("Timestep")
    plt.ylabel("Speed (m/s)")
